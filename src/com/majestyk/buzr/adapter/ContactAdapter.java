...
public class ContactAdapter extends BaseAdapter {
	...
	public ContactAdapter(Context c, LinkedList<Friend> list, OnContactClickListener oicl) {
		mContext = c;
		items = list;
		listener = oicl;
	}
	...
	public View getView(final int pos, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			case 0:
				final ContactItem friendItem = (ContactItem) items.get(pos);
					...
					public void onClick(View v) {						
						if (friendItem.getRegistered()) {
							if (friendItem.getFollowing()) {
								listener.onAre_FollowClick(friendItem, btn);
							} else if (!friendItem.getFollowing()) {
								listener.onNot_FollowClick(friendItem, btn);
							}
						} else if (!friendItem.getRegistered()) {
							if (friendItem.getFollowing()) { }
							else if (!friendItem.getFollowing()) {
								listener.onInviteUserClick(friendItem.getPhone());
							}
						}
					}
				});
				...
				if(friendItem.getImage().equals(""))
					holder.profPic.setImageResource(R.drawable.temp_profile_image_136);
				else
					holder.profPic.setImageURL(friendItem.getImage());
				holder.profName.setText(friendItem.getUsername());
